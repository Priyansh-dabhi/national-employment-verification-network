...
ORDERER_CA="$PROJECT_ROOT/../network/organizations/ordererOrganizations/nevs.gov/orderers/orderer.nevs.gov/msp/tlscacerts/tlsca.nevs.gov-cert.pem"
...
peer channel fetch config config_block.pb -o localhost:7050 -c nevs-channel --tls --cafile "$ORDERER_CA"
...
# Since CentralGovt is the only original org, its admin signature is enough.
peer channel update -f config_update_envelope.pb -o localhost:7050 -c nevs-channel --tls --cafile "$ORDERER_CA"

echo "Waiting for orderer to process org addition..."
sleep 5

echo "========== Step 14: Join CompanyOrg Peer to Channel =========="
# Switch identity to CompanyOrg Admin
...
peer channel fetch 0 nevs-channel.block -o localhost:7050 -c nevs-channel --tls --cafile "$ORDERER_CA"
...
# The printOrg in step 10 MIGHT have injected the anchor peer, but doing an explicit update ensures it is set.
peer channel fetch config company_config_block.pb -o localhost:7050 -c nevs-channel --tls --cafile "$ORDERER_CA"
...
# Extract anchor peers specifically
...
# Note: Anchor peer updates must be signed by the org's own admin, which we are right now.
peer channel update -f anchor_update_envelope.pb -o localhost:7050 -c nevs-channel --tls --cafile "$ORDERER_CA"
